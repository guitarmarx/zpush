...
sed -i "s/('TIMEZONE', '')/('TIMEZONE', '$TIMEZONE')/g" $common_config
sed -i "s/('USE_FULLEMAIL_FOR_LOGIN'.*/('USE_FULLEMAIL_FOR_LOGIN', 'true');/g" $common_config
sed -i "s/('IPC_PROVIDER'.*/('IPC_PROVIDER', 'IpcSharedMemoryProvider');/g" $common_config
sed -i "s/('BACKEND_PROVIDER'.*/('BACKEND_PROVIDER', '$BACKEND_PROVIDER');/g" $common_config
